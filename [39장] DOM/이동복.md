...
HTML 문서를 렌더링 엔진이 파싱한다면 렌더링 엔진은 위 HTML 문서를 파싱하여 DOM을 생성한다.
...
이와 같이 DOM은 노드 객체의 계층적인 구조로 구성된다. 
노드 객체에는 종류가 있고 상속 구조를 갖는다 노드 객체는 총 12개의 종류(노드 타입)가 있다. 
...
- 브라우저 환경의 모든 자바스크립트 코드는 분리된 script 태그라고해도 단 하나의 전역 객체 window를 공유한다.
- 모든 자바스크립트 코드는 전역 객체 window의 document 프로퍼티에 바인딩된 하나의 document를 바라본다
- DOM 트리의 루트 누드로 DOM 트리의 노드들에 접근을 위한 진입점 역할을 한다. 
...
노드 객체의 상속 구조는 개발자 도구 Elements - Properties 패널에서 확인 가능하다.
...
- 요소 노드를 우선적으로 취득하여야 HTML 구조나 내용, 스타일을 동적으로 조작할 수 있다.
- 텍스트 노드는 요소 노드의 자식 노드이고, 어트리뷰트 노드는 요소 노드와 연결되어 있어 텍스트 노드나 어트리뷰트 노드를 조작할 때도 마찬가지이다.
...
- getElementByld 메서드는 Document.prototpye의 프로퍼티이므로 Document 문서 노트를 반드시 호출하도록 한다.
- id 값은 HTML 문서 중 unique한 값으로 공백 문자로 구분해 여러 개의 값을 가질 수 없다.
- 중복된 id를 갖는 요소가 여러개 존재할 수 있는데 (에러 발생x) 이 경우 getElementByld의 인수인 d 값을 갖는 첫 요소 노드만 반환된다.
...
- Document.prototype.getfElementsByTagName 메서드는 DOM의 루프 노드인 문서 노드 (document)를 통해 호출하여 DOM 전체에서 요소 노드를 참색해 반환한다.
...
- getElementsByTagName 메서드와 같이 getElementsByclassName 메서드는 Documentprototype에 정의된 메서드와 Element.prototype에 정의된 메서드가 있다.
...
- 인수로 전달된 class 값을 갖는 요소가 없으면 getElerentsByClassName 메서드는 빈 HTMLCollection 객체를 반환한다.

### 39.2.4 CSS 선택자를 이용한 요소 노드 취득

- CSS 선택자는 스타일을 적용할 때 HTML 요소를 특정할 때 사용하는 문법을 말한다.

- Document.prototype/Element prototypequerySelector 서드는 인수로 전달한 CSS 선택자를 만족하는 하나의 요소 노드를 탐색하고 반환한다.
- 인수로 전달한 CSS 선택자를 만족하는 요소 노드가 여럿이면 첫 번째 요소 노트만 반환 인수로 전달된 CSS 선택자를 만족하는 요소 노드가 없으면 null 반환 인수로 전달한 CSS 선택자가 문법에 맞지 않으면 DOMException 에러가 발생
...
- HTMLCollection은 언제나 ive 객체(상태 변화 실시간 반영)로 등착한다.
- NodeList는 대부분 과거의 정적 상태를 유지하는 non-ive 객체로 등작하지만 때로는 live 객체로 동작한다.
...
3. ﻿﻿﻿className 프로퍼티를 사용해 모든 요소의 class 값을 'red'에서 "blue'로 변경 위 예제에서 I 요소의 class 값은 'blue'로 변경되고 모든 i 요소는 파란색으로 렌더링되어야 하지만 두번째 1 요소만 class 값이 변경되지 않는다.
...
3. 세 번째 반복(i === 2)
현재 $elems에는 두 번째 1 요소 노트만 남아 있어 $elems.length는 1이고 for문의는 i는 2이므로 반복문이 종료된다.
...
for문을 역으로 순회하여 이 문제를 해결할 수 있다.

또는 while문으로 HTMLCollection 객체에 노드 객체가 남지 않을 때까지 반복하여 해결할 수 있다.
...
HIM Colection을 사용하지 않음으로서 더 근본적으로 문제를 해결할 수 있다.
...
- query SelectorAIl 메서드는 DOM 컬렉션 객체 NodeList 객체를 반환한다.
- NodeList 객체는 실시간으로 노드 객체의 상태 변경을 반영하지 않는 객체이다.

코드 예제 39-22

- NodeList 객체는 NodeList.prototypeforEach 메서드를 상속받아 사용할 수 있다.
- NodeList의 forEach 메서드는 Array의 forEach와 사용법이 동일하다.
- NodeList 객체는 대부분의 경우 non-live 객체로 동작한다.
- 하지만 childNodes 프로퍼티가 반환하는 NodeList 객체는 HTMLCollection 객체와 같이 Iive 객체로 동작한다.

코드 예제 39-23

이처럼 HTMLCollection이나 Nodelist 객체는 예상과 다르게 동작하는 경우가 많다.
...
HTMLCollection과 NodeList 객체는 유사 배열 객체이면서 이터러블로 스프레드 문법이나 Array.trom 메서드를 사용해 배열 변환이 가능하다.

코드 예제 39-24

## 39.3 노드 탐색

- 요소 노드 취득 후 요소 노드를 기점으로 DOM 트리의 부모, 형제, 자식 노드 등을 탐색해보자 

코드 예제 39-25

- ul#fruits 요소는 3개의 자식 요소를 갖는다. 먼저 ul#furits 요소 노드 취득 후 하나 혹은 모든 자식 노드를 탐색할 수 있다.
- Ii.banana 요소는 2개의 형제 요소와 부모 요소를 갖는다.
- 먼저 li.banana 요소 노드를 취득하고 형제 노드 혹은 부모 노드를 탐색할 수 있다.
- 이처럼 DOM 트리 상의 노드를 탐색할 수 있게 Node, Element 인터페이스는 트리 탐색 프로퍼티를 제공한다.
...
- 위 HTML 문서에도 공백 문자가 포함되어 있으며 파싱되어 다음과 같은 DOM을 생성한다.
...
### 39.3.3 자식 노드 존재 확인
- ﻿﻿Node.prototypehasChildNodes 메서드를 사용한다.자식 노드가 존재하면 true 없으면 false를 반환한다.
...
![image](https://github.com/user-attachments/assets/05264f0e-98f1-4997-b2a6-a372d54c0206)
...
- 노트 객체의 nodeVaue 프로퍼티를 참조하면 노드 객체의 값을 반환한다.
- 노트 객체의 값은 텍스트 노드의 텍스트이다.
- 텍스트 노드가 아닌 문서 노드나 요소 노드의 nodeValue 프로퍼티를 참조하면 nul을 반환한다
...
- nodeValue 프로퍼티를 참조해도 텍스트 취득이 가능했지만 텍스트 노드가 아닌 노드면 null을 반환하므로 의미가 없다.
...
- innerText 프로퍼티는 CSs에 순종적으로 CSs에 의해 비표시(visibility: hiddeny)이면 지정 요소 노드 텍스트를 반환하지 않음.
